  ...
  switch (mode) {
    ...
    case "add":
      if (table == "goods" || table == "services") {
        return `INSERT INTO ${table} (name, notes, group_name, price_coins) VALUES \
							('${arr.name}',${arr.notes},'${arr.group_name}, ${arr.price_coins}')`;
      } else if (table == "todos") {
...
connectionToDB.connect(err => {
  if (err) throw err;
  console.log(
    `${new Date().getHours()}:${new Date().getMinutes()}  just connected to DB`
  );
});
          ...
          connectionToDB.query(
            ...
            (err, result) => {
              if (err) {
                ...
                res.write(
                  JSON.stringify("some error happen ask Serhii'co to solve it")
                );
                console.error(
